...
fn protocol_version_agrees_across_rust_dll_and_ea() {
    ...
    assert_eq!(
        define(&h, "MT5_BRIDGE_PROTOCOL_VERSION"),
        PROTOCOL_VERSION as u64,
        "DLL header"
    );
    assert_eq!(
        define(&ea, "PROTOCOL_VERSION"),
        PROTOCOL_VERSION as u64,
        "EA"
    );
}
...
fn packed_struct_sizes_agree_between_rust_and_dll_static_asserts() {
    ...
    for (name, rust_size) in expect {
        ...
        let line = h
            ...
            .unwrap_or_else(|| panic!("no static_assert for {name}"));
        let n: usize = line
            .split("== ")
            .nth(1)
            .unwrap()
...
fn deal_struct_field_order_is_identical_in_the_dll_header() {
    ...
    assert_eq!(
        (f64_at(48), f64_at(56), f64_at(64), f64_at(72), f64_at(80)),
...
fn ea_token_format_matches_the_rust_wire_id() {
    ...
    for excluded in ["'I'", "'L'", "'O'", "'U'"] {
        assert!(
            body.contains(&format!("c != {excluded}")),
            "EA must exclude {excluded}"
        );
    }
    ...
    assert!(
        !extract.contains("return cmt;"),
        "ExtractClientOrderId must not return the raw comment"
    );
...
fn dll_deals_request_layout_matches_the_ea_handler() {
    ...
    .iter()
    .map(|k| dll.find(k).unwrap_or_else(|| panic!("DLL missing {k}")))
    .collect();
    assert!(
        order.windows(2).all(|w| w[0] < w[1]),
        "DLL packs in the wrong order"
    );

    let h = &ea[ea.find("void HandleDealsGet(").unwrap()..];
    let h = &h[..h.find("\n}\n").unwrap()];
    let order: Vec<usize> = [
        "SafeUnpackI64(payload, off, len, from_utc)",
    ...
    .iter()
    .map(|k| h.find(k).unwrap_or_else(|| panic!("EA missing {k}")))
    .collect();
    assert!(
        order.windows(2).all(|w| w[0] < w[1]),
        "EA unpacks in the wrong order"
    );
    ...
    .iter()
    .map(|k| h.rfind(k).unwrap_or_else(|| panic!("EA missing {k}")))
    .collect();
    assert!(
        packs.windows(2).all(|w| w[0] < w[1]),
        "EA packs Mt5Deal fields in the wrong order: {packs:?}"
    );
}
